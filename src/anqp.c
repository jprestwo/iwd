...
bool anqp_iter_next(struct anqp_iter *iter)
{
					...
					unsigned int len, const char **nai_out)
{
	...
	strncpy(nai, (const char *)anqp, nai_len);
	anqp += nai_len;
...
bool anqp_parse_vendor_wfa(const unsigned char *anqp, unsigned int len,
				uint8_t *stype, const unsigned char **vdata,
				unsigned int *vlen)
{
	uint8_t type;
	...
	if (type != 0x11) {
		l_error("Expected type 0x11, got 0x%02x", type);
		return false;
	}
...
static bool parse_eap_params(const unsigned char *anqp,
					unsigned int len,
					uint8_t *method,
			...
			struct anqp_eap_method *method_out)
{
	...
	while (eap_count--) {
		...
		l_strlcpy(method_out->realm, nai, strlen(nai) + 1);
				...
				bool hs20, struct anqp_eap_method *method_out)
{
	...
	while (count--) {
		...
		l_strlcpy(nai_realm, (char *)anqp + 2, nai_len + 1);
		...
		/* suitable method not found, advance to next */
		if (len < realm_len)
			return false;
