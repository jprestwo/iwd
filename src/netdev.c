...
static int netdev_associate(struct handshake_state *hs)
{
	struct netdev *netdev;
	struct l_genl_msg *msg;

	netdev = netdev_find(hs->ifindex);

	msg = netdev_build_cmd_associate_common(netdev);

	l_genl_msg_append_attr(msg, NL80211_ATTR_IE, hs->own_ie[1] + 2,
							hs->own_ie);
...
static void netdev_sae_failed(uint32_t ifindex, uint16_t reason, void *data)
{
	struct netdev *netdev = netdev_find(ifindex);

	l_error("SAE exchange failed on %u reason %u", ifindex, reason);

	/* sm gets freed by sae */
	netdev->sae_sm = NULL;
				...
				const char *whitelist, const char *blacklist)
{
	...
	__sae_set_associate_func(netdev_associate);
	__sae_set_failed_func(netdev_sae_failed);
