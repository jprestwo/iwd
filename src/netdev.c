					...
					void *user_data)
{
	...
	/*
	 * Only fail the connection if the interface is still down. This failure
	 * could be the result of either setting the MAC OR bringing the iface
	 * up since they were done in the same operation. If the interface is
	 * up but we still got an error just log it and continue.
	 */
	if (error && !netdev_get_is_up(netdev)) {
		...
		l_error("Error changing the MAC but interface %d is still up, "
			"continuing", netdev->index);
