...
typedef void (*sae_frame_watch_func_t)(const uint8_t *from,
...
static sae_tx_packet_func_t tx_packet;
static sae_associate_func_t associate_func;
static sae_failed_func_t failed_func;
...
static bool sae_frame_watch_match_ifindex(const void *a, const void *b)
{
	struct sae_frame_watch *sfw =
		container_of(a, struct sae_frame_watch, super);

	return sfw->ifindex == L_PTR_TO_UINT(b);
}
				...
				const uint8_t *frame, size_t len)
{
	WATCHLIST_NOTIFY_MATCHES(&frame_watches,
					sae_frame_watch_match_ifindex,
					L_UINT_TO_PTR(ifindex),
					sae_frame_watch_func_t, src,
					frame, len);
}

void __sae_tx_packet(uint32_t ifindex, const uint8_t *dst,
			const uint8_t *frame, size_t len)
{
	if (!tx_packet)
		return;

	tx_packet(ifindex, dst, frame, len, tx_user_data);
}

static void sae_frame_watch_free(struct watchlist_item *item)
{
	struct sae_frame_watch *sfw =
		container_of(item, struct sae_frame_watch, super);

	l_free(sfw);
}

static const struct watchlist_ops sae_frame_watch_ops = {
	.item_free = sae_frame_watch_free,
};

static uint32_t sae_frame_watch_add(uint32_t ifindex,
				sae_frame_watch_func_t handler,
				void *user_data)
{
	struct sae_frame_watch *sfw;

	sfw = l_new(struct sae_frame_watch, 1);
	sfw->ifindex = ifindex;

	return watchlist_link(&frame_watches, &sfw->super,
				handler, user_data, NULL);
}

static void sae_frame_watch_remove(uint32_t id)
{
	watchlist_remove(&frame_watches, id);
}
...
static bool sae_pwd_seed(const uint8_t *addr1, const uint8_t *addr2,
		uint8_t *base, size_t base_len, uint8_t counter, uint8_t *out)
{
	uint8_t key[12];
...
{
	if (failed_func)
		failed_func(sm->handshake->ifindex, reason, tx_user_data);
...
static void sae_reject_authentication(struct sae_sm *sm, uint16_t reason)
{
	...
	__sae_tx_packet(sm->handshake->ifindex, sm->handshake->aa,
				reject, ptr - reject);

	l_timeout_remove(sm->retransmit);
	sm->retransmit = NULL;
...
static void sae_send_confirm(struct sae_sm *sm)
{
	...
	__sae_tx_packet(sm->handshake->ifindex, sm->handshake->aa, body, 38);
		...
		const uint8_t *frame, size_t len)
{
	...
	if (associate_func)
		associate_func(sm->handshake->ifindex);
...
static void sae_send_commit(struct sae_sm *sm, bool retry)
{
	...
	__sae_tx_packet(hs->ifindex, hs->aa, commit, len);

	sm->state = SAE_STATE_COMMITTED;
}

static void sae_set_retransmit_timer(struct sae_sm *sm);

static void sae_send_commit_retry(struct l_timeout *timeout, void *data)
{
	struct sae_sm *sm = data;
	...
	default:
		/* should never happen */
		l_error("retransmit timer fired in bad state %u", sm->state);
		return;
	}

	sae_set_retransmit_timer(sm);
}

static void sae_set_retransmit_timer(struct sae_sm *sm)
{
	sm->retransmit = l_timeout_create(SAE_RETRANSMIT_TIMEOUT,
						sae_send_commit_retry, sm,
						NULL);
}
					...
					size_t len)
{
	...
	/* timer is reset regardless of state */
	l_timeout_remove(sm->retransmit);
	sm->retransmit = NULL;

	/*
	 * Upon receipt of a Con event, the t0 (retransmission) timer shall be
	 * canceled. Then the protocol instance checks the value of Sync. If it
	...
	if (transaction == SAE_STATE_CONFIRMED) {
		if (sm->sync > SAE_SYNC_MAX) {
			/* send del event and transition to nothing */
			sm->state = SAE_STATE_NOTHING;
			return false;
		}
					...
					size_t len)
{
	...
	/*
	 * If the Status is nonzero, the frame shall be silently discarded, the
	 * t0 (retransmission) timer set, and the protocol instance shall remain
...
static void sae_rx_packet(const uint8_t *from, const uint8_t *frame,
				size_t len, void *user_data)
{
	struct sae_sm *sm = user_data;
...
void sae_register(struct sae_sm *sm)
{
	...
	sm->watch_id = sae_frame_watch_add(sm->handshake->ifindex,
						sae_rx_packet, sm);
...
static void sae_sm_destroy(void *data)
{
	struct sae_sm *sm = data;

	sae_frame_watch_remove(sm->watch_id);
...
void sae_init(void)
{
	watchlist_init(&frame_watches, &sae_frame_watch_ops);
}

void sae_exit(void)
{
	if (!l_queue_isempty(state_machines))
		l_warn("stale sae state machines found");

	l_queue_destroy(state_machines, sae_sm_destroy);
