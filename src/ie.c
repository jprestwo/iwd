				...
				uint32_t chan_width, uint8_t *data_rate)
{
	...
	if (chan_width == IE_BSS_CHAN_WIDTH_10)
		width_adjust = 3;
	else if (chan_width == IE_BSS_CHAN_WIDTH_5)
		width_adjust = 6;
	...
	for (i = L_ARRAY_SIZE(rate_rssi_map) - 1; i >= 0; i--) {
		...
		if (rssi < map->rssi - width_adjust)
			continue;

		for (j = 0; j < len; j++) {
			printf("RATE: %u\n", rates[j]);
			printf("COMPARE: %u\n", map->rate);
			if ((rates[j] & 0x7f) == map->rate) {
				*data_rate = map->rate;
				return 0;
			}
		}
				...
				uint8_t *data_rate)
{
	...
	l_debug("data[0]=%u, data[1]=%u\n", data[0], data[1]);

	ie_tlv_iter_init(&iter, data, len);

	l_debug("iter.data=%u, iter.len=%u, iter.tag=%u\n", iter.data[0], iter.len, iter.tag);

	if (!ie_tlv_iter_next(&iter)) {
		l_debug("could not advance iterator\n");
		return -EMSGSIZE;
	}
	...
	return ie_parse_supported_rates(&iter, rssi, chan_width, data_rate);
}
 ...
 */
static int32_t ht_base_rssi[] = { -82, -79, -77, -74, -70, -66, -65, -64 };
...
static struct ht_vht_rate ht_vht_rates[] = {
				...
				uint64_t *data_rate)
{
	...
	struct ht_vht_rate *rate;

	l_debug("MCS: %u, RSSI: %d, 40MHZ: %u, SGI: %u", index, rssi, support_40mhz, short_gi);

	if (rssi < ht_base_rssi[index % 8] + width_adjust)
		return false;

	/* Found an acceptable MCS index */

	if (support_40mhz)
		rate = &ht_vht_rates[HT_VHT_CHANNEL_WIDTH_40MHZ];
	else
		rate = &ht_vht_rates[HT_VHT_CHANNEL_WIDTH_20MHZ];
...
/* Base RSSI values for 20MHz VHT channel */
static int32_t vht_base_rssi[] = {
 ...
 */
static uint8_t vht_width_map[3][5] = {
	[0] = {1, 1, 1, 0, 0 },
	[1] = {1, 1, 1, 1, 0 },
	[2] = {1, 1, 1, 1, 1 },
};
				...
				uint64_t *data_rate)
{
	...
	if (ie_tlv_iter_get_tag(iter) != IE_TYPE_VHT_CAPABILITIES)
		return -EINVAL;

	data = ie_tlv_iter_get_data(iter);

	printf("data[0]=%02x, data[3]=%02x\n", data[0], data[3]);

	channel_width_set = util_bit_field(*data, 2, 2);

	data += 4;

	ext_nss_bw = util_bit_field(*data, 6, 2);

	l_debug("channel width set %u\n", channel_width_set);
	l_debug("ext nss bw %u", ext_nss_bw);
	...
	for (i = 15; i >= 0; i -= 2) {
		uint8_t rx_val = util_bit_field(rx_mcs_map[i / 8], i % 8, 2);
		uint8_t tx_val = util_bit_field(tx_mcs_map[i / 8], i % 8, 2);
		 ...
		 *
		 * Therefore 7 + rx/rx_val gives us our max MCS index.
		 */
	...
	*data_rate = 0;
	 ...
	 */
	for (k = sizeof(vht_width_map[0]) - 1; k >= 0; k--) {
		if (!vht_width_map[channel_width_set][k])
			continue;

		rate = &ht_vht_rates[k];
		width_adjust = k * 3;

		for (j = 1; j <= MIN(rx_nss, tx_nss); j++) {
			for (i = MIN(max_rx_mcs, max_tx_mcs); i >= 0; i--) {
				uint64_t drate;

				if (rssi < vht_base_rssi[i] + width_adjust)
					continue;
				...
				if (drate > *data_rate) {
					l_debug("Found higher data rate with MCS: %u, NSS: %u, width: %u\n", i, j, k);
					*data_rate = drate;
				}
			}
		}
					...
					int32_t rssi, uint64_t *data_rate)
{
	...
	return ie_parse_vht_capability(&iter, rssi, data_rate);
}
