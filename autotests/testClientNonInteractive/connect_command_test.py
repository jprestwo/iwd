...
class Test(unittest.TestCase):
    ...
    def test_connection_with_passphrase(self):
        ...
        ctx.start_process(['iwctl', '-P', 'passphrase', 'station', device.name, 'connect', ssid], check=True)
    ...
    def test_connection_with_username_and_password(self):
        ...
        #                        'station', device.name, 'connect', ssid])
        ctx.start_process(['iwctl', '-u', 'user', '-p', 'password', 'station', device.name, 'connect', ssid], check=True)
        self.check_connection_success(ssid)
    ...
    def test_connection_with_password(self):
        ...
        #                        'station', device.name, 'connect', ssid])
        ctx.start_process(['iwctl', '-p', 'password', 'station', device.name, 'connect', ssid], check=True)
    ...
    def test_connection_failure(self):
        ...
        with self.assertRaises(subprocess.CalledProcessError):
                ...
                #                'station', device.name, 'connect', ssid])
                ctx.start_process(['iwctl', '-P', 'incorrect_passphrase', 'station', device.name, 'connect', ssid], check=True)
